...
<script type="text/javascript">
	...
	function makeView(data) { // data=[{    }, {    }, {    },,,  ]
		...
		$
				.each(
						data,
						function(index, obj) { // obj={"idx": 5, "title": "게시판" ~~~}
							listHtml += "<tr>";
	...
	function goForm() {
		...
		$("#wForm").css("display", "block"); // 보여주고
	}
	function goList() {
		$("#view").css("display", "block"); // 보여주고
	...
	function goContent(idx) { // idx=11, 10, 9
		if ($("#c" + idx).css("display") == "none") {
			$.ajax({
				...
				type : "get",
				dataType : "json",
				success : function(data){	// data = { "contnet" : ~~!!~# }
					$("#ta"+idx).val(data.content);
				},	
				error : function(){ alert("goContent error");}
			});
			$("#c" + idx).css("display", "table-row"); // 보여주고
			$("#ta" + idx).attr("readonly", true);
		} else {
			$("#c" + idx).css("display", "none"); // 감추고
			$.ajax({
				url : "board/count/"+idx,
				type : "put",
				dataType : "json",
	...
	function goUpdateForm(idx) { // idx= 10, 7, 8
		...
		$("#t" + idx).html(newInput);
		var newButton = "<button class='btn btn-primary btn-sm' onclick='goUpdate("
				+ idx + ")'>수정</button>";
...
<jsp:include page="../common/header.jsp"/>
	<div class="container">
		...
		<div class="panel panel-default">
			...
			<div class="panel-footer">Surable Technology</div>
		</div>
	</div>
